...
def index(req):
	 ...
	 if "Bonafide Certificate For" in docm:
		docm1=docm.split('For')
			
      		docm1=docm1[0].strip()
		
         	ss="""select  appId from applicationProcess.applicationDomain where appDesc='%s';"""%(docm1)
         	cursor.execute(ss)   
		...
		porps=docm.split("Bonafide Certificate For")
		porps=porps[1].strip()
         	...
         	ss= """select * from applicationProcess.applicationFormForStaff where rollNumber=%s and appId=%s and applicationPurpose=%s;"""
         	cursor.execute(ss,(Rollno,app,purpose))
		...
		ss4="""select courseId  from  studentDetailsDomain where rollNumber=%s;"""%(Rollno)
	        ...
	        im = Image(logo, 1*inch, 1*inch)
                ...
                year=val[0][2]
                year=int(math.ceil(float(year)/2))
                dt1=dt.split("/");
	        ...
	        ptext = '<para alignment=\"JUSTIFY\"><font size=12>This is to certify that %s %s %s %s(Roll No.%s ) is a regular student of %s,course (year %s) %s at the Department of Computer Science, Savitribai Phule,Pune University.This Certificate has been issued to the student on  %s request for the purpose of %s.As per record of this Department,the other details of the student are as follows:-</font></para>' %(g,fn,mn,ln,rollno,cource,yy[0],year1,hh,purpose,)
                Story.append(Paragraph(ptext,styles["Normal"])) 
                Story.append(Spacer(1, 30))
                
                ptext = '<html><font size=12>Address:%s</font></html>'%(addl1) 
	        Story.append(Paragraph(ptext, styles["Normal"]))
                ptext = '<html>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<font size=12>%s</font></html>'%(addl2)
 		Story.append(Paragraph(ptext, styles["Normal"])) 
                ptext = '<html>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<font size=12>%s</font></html>'%(addl3)
	        Story.append(Paragraph(ptext, styles["Normal"])) 
	        ...
	        Story.append(Paragraph(ptext, styles["Normal"]))   
                ...
                fp=open(data.path+"/project_data/doc_pdf/"+filename)	
 		fp=fp.read() 
                return fp
		 ...
		 ss5="""select category from studentDetailsDomain where rollNumber=%s;"""%(Rollno)
	    	 ...
	    	 if val[0][4]=='mca':  
	           ...
	           ptext = '<para alignment=\"JUSTIFY\"><html>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<font size=12>This letter has been issued on the basis of request by the student for the Purpose of getting Professional Education loan from your Bank.</font></html></para>'
	           Story.append(Paragraph(ptext, styles["Normal"])) 
                   Story.append(Spacer(1, 10))
                   ptext = '<para alignment=\"JUSTIFY\"><html>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<font size=12>We, hereby certify that %s %s %s %s is a bonafide student of this department studying in %s year of %s the said course is of 3 year\'s duration.</font></html></para>'%(g,fn,mn,ln,year,cource)
	           Story.append(Paragraph(ptext, styles["Normal"])) 
                   Story.append(Spacer(1, 10))
                   ptext = '<para alignment=\"JUSTIFY\"><html><font size=12><u>The minimum fees for Maharashtra Domicile student belonging %s Category are as detailed below. (The tuition & Lab fees may increase as per the university rules & norms)</u> </font></html></para>'%(categary)
	           Story.append(Paragraph(ptext, styles["Normal"])) 
	           ...
	           t2.setStyle(style)
                   Story.append(t2)
                   Story.append(Spacer(1, 20))
                   ptext = '<para alignment=\"JUSTIFY\"><html><font size=12><b>The amount towards fees can be paid through the Demand Draft to be in favour of \'Finance & Accounts Officer, Savitribai Phule Pune University\',Payable at Pune.</b> </font></html></para>'
	           Story.append(Paragraph(ptext, styles["Normal"])) 
                   Story.append(Spacer(30, 40)) 
	           ...
	           doc12.build(Story)
                   ...
                   fp=open(data.path+"/project_data/doc_pdf/"+filename,"r"); 
	           	...
	           	Story.append(Paragraph(ptext, styles["Normal"]))
	                ...
	                Story.append(Paragraph(ptext, styles["Normal"])) 
                        Story.append(Spacer(1, 10))
                        ptext = '<para alignment=\"JUSTIFY\"><html>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<font size=12>We, hereby certify that %s %s %s %s is a bonafide student of this department studying in %s  year of %s the said course is of 2 year\'s duration.</font></html></para>'%(g,fn,mn,ln,year,cource)
	                Story.append(Paragraph(ptext, styles["Normal"])) 
                        Story.append(Spacer(1, 10))
                        ptext = '<para alignment=\"JUSTIFY\"><html><font size=12><u>The minimum fees for Maharashtra Domicile student belonging %s Category are as detailed below. (The tuition & Lab fees may increase as per the university rules & norms)</u> </font></html></para>'%(categary)
	                Story.append(Paragraph(ptext, styles["Normal"])) 
         ...
         if app=="APND":
	        ...
	        magName = "Pythonista"
                ...
                dt=d
                g=gen
                nm = val[0][4].upper()+" "+val[0][5].upper()+" "+val[0][6].upper()
                rollno=Rollno
                cource=cname
	        ...
	        Story.append(Spacer(1, 12))
                ...
                ptext = '<para alignment=\"JUSTIFY\"><font size=11>%s %s Roll No.%s.Is leaving the %s (Computer Science) Programs Please recover any dues, outstanding  against %s and give %s no dues.</font>' %(g,nm,rollno,cource,hh,on_print_gender)
                Story.append(Paragraph(ptext,styles["Normal"])) 
	        Story.append(Spacer(40, 50))
		...
		ss = """select * from applicationProcess.studentDetailsDomain where rollNumber=%s ;"""%(Rollno)
	        ...
	        Story.append(Spacer(1, 20))
                ...
                ptext = '<para alignment=\"JUSTIFY\"><html><font size=12>This is to certify that %s %s %s is a bonafide student of this department. His particulars as per our record are as follows:</font></html></para>'%(fn,mn,ln)
                Story.append(Paragraph(ptext, styles["Normal"])) 
                Story.append(Spacer(1, 30))
